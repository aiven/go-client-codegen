...
const (
	genImport       = "github.com/aiven/aiven-go-client-v3"
...
func exec() error {
	...
	docBytes, err := os.ReadFile(openapiFile)
	if err != nil {
		return err
	}
	...
	client := jen.NewFile(clientModule)
	...
	if s.isEnum() {
		...
		for _, e := range s.Enum {
			...
			if !reMakesSense.MatchString(literal) {
				// sometimes there are invalid enums, for instance just a comma ","
				continue
			}
